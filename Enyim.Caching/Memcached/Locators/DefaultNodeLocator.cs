...
namespace Enyim.Caching.Memcached
{
	...
	{
		private const int _serverAddressMutations = 100;
		...
		{
			uint[] keys = new uint[nodes.Count * _serverAddressMutations];

			int nodeIdx = 0;

			foreach (IMemcachedNode node in nodes)
			{
				uint[] tmpKeys = GenerateKeys(node, _serverAddressMutations);
		...
		private static uint[] GenerateKeys(IMemcachedNode node, int numberOfKeys)
		{
			...
			uint[] k = new uint[PartCount * numberOfKeys];
